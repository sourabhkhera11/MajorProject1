...
export class OrderController extends BaseController {
  static async createOrder(ctx: Context) {
    return OrderController.execute(
      ctx,
      async () => {
        ...
        const { numberOfUnitsOrdered, totalAmount, variantId, customerId } =
          orderData;
        ...
        if (!numberOfUnitsOrdered || numberOfUnitsOrdered < 1) {
          throw new AppError(
            "Number of units ordered is required!",
    ...
    return OrderController.execute(
      ...
      async () => {
        ...
        const statusCancelable = [
          OrderStatus.CONFIRED,
          OrderStatus.SHIPPED,
          OrderStatus.OUT_FOR_DELIVERY,
        ];

        if (
          status === OrderStatus.CONFIRED ||
          existingStatus === OrderStatus.REFUNDED ||
          existingStatus === OrderStatus.CANCELLED ||
          (status === OrderStatus.SHIPPED &&
            existingStatus != OrderStatus.CONFIRED) ||
          (status === OrderStatus.OUT_FOR_DELIVERY &&
            existingStatus != OrderStatus.SHIPPED) ||
          (status === OrderStatus.DELIVERED &&
            existingStatus != OrderStatus.OUT_FOR_DELIVERY) ||
          (status === OrderStatus.CANCELLED &&
            !statusCancelable.includes(existingStatus)) ||
          (status === OrderStatus.RETURNED &&
            existingStatus != OrderStatus.DELIVERED) ||
          (status === OrderStatus.REFUNDED &&
            existingStatus != OrderStatus.RETURNED)
        ) {
