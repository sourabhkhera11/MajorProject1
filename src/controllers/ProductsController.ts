...
export class ProductController extends BaseController {
    ...
    return ProductController.execute(
      ...
      async () => {
        ...
        const tags = ctx.query.tags
          ? (ctx.query.tags as string).split(",")
          : ([] as string[]);
  ...
  static async totalSales(ctx: Context) {
    return ProductController.execute(ctx, async () => {
      ...
      const numberOfProducts = data.orders.reduce(
        (num, product) => num + Number(product.numberOfUnitsOrdered),
        0,
      );
      const totalSales = data.orders.reduce(
        (amount, product) => amount + Number(product.totalAmount),
        0,
      );
      return {
