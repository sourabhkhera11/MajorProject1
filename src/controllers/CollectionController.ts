...
export class CollectionController extends BaseController {
  static async createCollection(ctx: Context) {
    return CollectionController.execute(
      ctx,
      async () => {
    ...
    return CollectionController.execute(
      ...
      async () => {
        ...
        const fields = ctx.query.fields
          ...
          : ([] as string[]);
        const collections = await collectionRepo.fetchCollections(
          take,
          skip,
          fields,
        );
    ...
    return CollectionController.execute(
      ...
      async () => {
        const fields = ctx.query.fields
          ? (ctx.query.fields as string).split(",")
          : ([] as string[]);
        const collection = await collectionRepo.fetchCollection(
          ctx.params.id,
          fields,
        );
        if (!collection) {
    ...
    return CollectionController.execute(
      ...
      async () => {
        ...
        const existingProduct =
          await productRepo.fetchFromProductArray(productIds);
        ...
        const result = await collectionRepo.addProductToCollection(
          collection,
          productsToAdd,
        );
        return {
